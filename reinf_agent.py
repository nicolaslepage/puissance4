# -*- coding: utf-8 -*-
"""
Created on Sat Apr 24 20:10:03 2021

@author: Windows
"""
import numpy as np
import matplotlib.pyplot as plt
from jeu import ai_moves
import random as rd
...
class ActionValueNetwork:
    ...
    def __init__(self, network_config):
        ...
        self.layer_sizes =np.zeros((self.num_hidden_layers), dtype = int)
    ...
    def get_action_values_l_layers(self, s):
        """
        Args:
            s (Numpy array): The state.
        Returns:
        ...
        for i in range (self.num_hidden_layers):
            W.append(self.weights[i]['W'])
            b.append(self.weights[i]['b'])
            
        
        psi = np.dot(s, W[0]) + b[0]
        x = np.maximum(psi, 0)
        
        for i in range (1,self.num_hidden_layers-1):
            psi = np.dot(x, W[i]) + b[i]
            x = np.maximum(psi, 0)
        
        q_vals = np.dot(x, W[self.num_hidden_layers-1]) + b[self.num_hidden_layers-1]
    ...
    def get_TD_update_l_layers(self, s, delta_mat,numlayers):
        """
        Args:
            s (Numpy array): The state.
            delta_mat (Numpy array): A 2D array of shape (batch_size, num_actions). Each row of delta_mat  
...
class Agent():
    ...
    def agent_step4(self, reward, state, real_state):
        """A step taken by the agent.
        Args:
            reward (float): the reward received for taking the last action taken
            state (Numpy array): the state from the
        ...
        while check :
            ...
            check = ai_moves(real_state,action,0)[0,0] == -1
